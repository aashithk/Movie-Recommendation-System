function [A,Y,numIter,tElapsed,finalResidual]=wnmfrule4(X,k,lamda,option)
...
for i=1:option.iter
    ...
    if mod(i,10)==0 || i==option.iter
        ...
        %curRes=norm(W.*(X-XfitThis),'fro')+(lamda*(sum(sum(A.^2))+sum(sum(Y.^2))));
        curRes=sum(sum(W.*((X-XfitThis).^2)))+(lamda*(sum(sum(A.^2))+sum(sum(Y.^2))));
