function [A,Y,numIter,tElapsed,finalResidual]=wnmfrule5v2(X,k,lambda,option)
...
for i=1:option.iter
    switch option.distance
        case 'ls'
            %A(u,:) = (Y*usermat{u}*TransY + lambdaI) \(Y * usermat{u} * TransX(:,u));
            A=A.*(((W.*X)*Y')./(lambda*A + (W.*(A*Y))*Y'));
            A=max(A,eps);
            %Y(:,m) = (AT*moviemat{m}*A + lambdaI)\  (AT*moviemat{m}*X(:,m));
    ...
    if mod(i,10)==0 || i==option.iter
        ...
        %fitRes=sqrt(sum(sum(W.*((XfitPrevious-XfitThis).^2))));
        fitRes=sum(sum(W.*((XfitPrevious-XfitThis).^2)));
        XfitPrevious=XfitThis;
        %curRes=norm(W.*(X-XfitThis),'fro');
        %curRes=sqrt(sum(sum(W.*((X-XfitThis).^2)))) + lambda * (sumsqr(A) + sumsqr(Y));
        curRes=sum(sum(W.*((X-XfitThis).^2))) + lambda * (sum(sum((A.^2))) +sum(sum((Y.^2))));
